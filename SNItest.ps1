...
Write-Host ""
Write-Host "SNI Ping & Speed Test v2" -ForegroundColor Cyan
    ...
    for ($i = 1; $i -le $pingAttempts; $i++) {
        ...
        foreach ($endpoint in $testEndpoints) {
            try {
                ...
                if ($saveErrorLog) { 
                    # Use mutex for thread-safe logging in parallel
                    $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                    $mutex.WaitOne() > $null
                    "Ping attempt failed for ${sniHost}${endpoint}: $_" | Out-File -FilePath $errorLogFile -Append -Encoding UTF8
                    $mutex.ReleaseMutex()
                }
                continue
            }
...
function Test-Speed {
    ...
    foreach ($endpoint in $testEndpoints) {
        ...
        try {
            ...
            if ($totalElapsed -gt 0 -and $totalData -gt 0) {
                ...
                if ($totalData -ge 1024) {
                    ...
                    if ($saveErrorLog) { 
                        $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                        $mutex.WaitOne() > $null
                        "Success for $endpoint (repeats: $rep, size: $totalData bytes, time: $totalElapsed s)" | Out-File -FilePath $errorLogFile -Append -Encoding UTF8
                        $mutex.ReleaseMutex()
                    }
                    break
                } else {
            ...
            if ($saveErrorLog) { 
                $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                $mutex.WaitOne() > $null
                "Speed test failed for ${sniHost}${endpoint}: $_" | Out-File -FilePath $errorLogFile -Append -Encoding UTF8
                $mutex.ReleaseMutex()
            }
            continue
        } finally {
    ...
    if ($downloadSpeed -eq 0) {
        if ($saveErrorLog) { 
            $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
            $mutex.WaitOne() > $null
            "No valid speed data for $sniHost" | Out-File -FilePath $errorLogFile -Append -Encoding UTF8
            $mutex.ReleaseMutex()
        }
    }
...
if ($PSVersionTable.PSVersion.Major -ge 7) {
    $results = $hosts | ForEach-Object -Parallel {
            ...
            for ($i = 1; $i -le $using:pingAttempts; $i++) {
                ...
                foreach ($endpoint in $testEndpoints) {
                    try {
                        ...
                        if ($using:saveErrorLog) { 
                            $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                            $mutex.WaitOne() > $null
                            "Ping attempt failed for ${sniHost}${endpoint}: $_" | Out-File -FilePath $using:errorLogFile -Append -Encoding UTF8
                            $mutex.ReleaseMutex()
                        }
                        continue
                    }
        ...
        function Test-Speed {
            ...
            foreach ($endpoint in $testEndpoints) {
                ...
                try {
                    ...
                    if ($totalElapsed -gt 0 -and $totalData -gt 0) {
                        ...
                        if ($totalData -ge 1024) {
                            ...
                            if ($using:saveErrorLog) { 
                                $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                                $mutex.WaitOne() > $null
                                "Success for $endpoint (repeats: $rep, size: $totalData bytes, time: $totalElapsed s)" | Out-File -FilePath $using:errorLogFile -Append -Encoding UTF8
                                $mutex.ReleaseMutex()
                            }
                            break
                        } else {
                    ...
                    if ($using:saveErrorLog) { 
                        $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                        $mutex.WaitOne() > $null
                        "Speed test failed for ${sniHost}${endpoint}: $_" | Out-File -FilePath $using:errorLogFile -Append -Encoding UTF8
                        $mutex.ReleaseMutex()
                    }
                    continue
                } finally {
            ...
            if ($downloadSpeed -eq 0) {
                if ($using:saveErrorLog) { 
                    $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                    $mutex.WaitOne() > $null
                    "No valid speed data for $sniHost" | Out-File -FilePath $using:errorLogFile -Append -Encoding UTF8
                    $mutex.ReleaseMutex()
                }
            }
        ...
        try {
            ...
            if ($using:saveErrorLog) { 
                $mutex = New-Object System.Threading.Mutex $false, "ErrorLogMutex"
                $mutex.WaitOne() > $null
                "DNS fail for ${sniHost}: $_" | Out-File -FilePath $using:errorLogFile -Append -Encoding UTF8
                $mutex.ReleaseMutex()
            }
        ...
        if ($using:testSpeed -and $connectionType -ne "TIMEOUT" -and $avgPing -le $using:maxPing) {
            ...
            if ($downloadSpeed -eq 0 -and $avgPing -gt 0 -and $avgPing -lt 1000) {
                # Arbitrary estimate based on ping; use with caution as it's not real speed
    ...
    $checkedHosts = 0
    # Use ArrayList to avoid O(n^2) on += for large host lists
    ...
    foreach ($sniHost in $hosts) {
        ...
        try {
            ...
            Write-Host ""
            if ($saveErrorLog) { 
                "DNS fail for ${sniHost}: $_" | Out-File -FilePath $errorLogFile -Append -Encoding UTF8
            }
